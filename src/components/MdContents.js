...
const MdContents = ({ html, lastUpdated }) => {
	useEffect(() => {
		...
		const insertReadTime = () => {
			...
			try {
				...
				const updatedText = lastUpdated
					? `<span class="last-updated-text">Updated: ${new Date(lastUpdated).toLocaleDateString('en-US', {
							year: 'numeric',
							month: 'short',
							day: '2-digit',
					  })}</span>`
				...
				try {
					...
					if (placeholder && FocusButton) {
						try {
							const root = createRoot(placeholder);
							root.render(React.createElement(FocusButton, null));
							addCleanup(() => {
								try {
		...
		const initHeaderClicks = () => {
			const mdCnt = document.getElementById('md-cnt') || document.getElementById('md-content');
			if (!mdCnt) {
				console.warn('MdContents: no #md-cnt or #md-content container found');
				return;
			}
			...
			if (typeof document !== 'undefined') {
				...
				if (!at) {
					...
					addCleanup(() => {
						const el = document.getElementById('anchor_tooltip');
			...
			heads.forEach((h) => {
				...
				if (anchor) {
					...
					const onAnchorMouseMove = (ev) => {
						lastMouseEvent = ev;
						showTip('Copy link', ev);
					};
			...
			addCleanup(() => handlers.forEach((fn) => fn && fn()));
		};
		...
		const initInlineCodeTabs = () => {
			...
			headings.forEach((h) => {
				if (++processed > MAX_PROCESS) return;
				...
				if (blocks.length !== liEls.length) return;
		...
		const initImageZoom = () => {
			...
			if (document.getElementById('zoom-image-container')) return;

			const imageZoom = `<div id="zoom-image-container"><div id="zoom-image-center"><span id="zoom-image-close">&#10006;</span><img id="zoom-image"></div></div>`;
			...
			const zoomImageClose = document.getElementById('zoom-image-close');
			const images = document.querySelectorAll('#md-content img:not(.no-zoom)');
			if (!zoomImageContainer || !zoomImage || !zoomImageClose) return;
			...
			const onOverlayClick = (event) => {
				if (event.target === zoomImageContainer) closeZoom();
			};
			...
			Array.from(images).forEach((img) => {
				...
				addCleanup(() => img.removeEventListener('click', onClick));
			});

			zoomImageClose.addEventListener('click', closeZoom);
			zoomImageContainer.addEventListener('click', onOverlayClick);

			addCleanup(() => zoomImageClose.removeEventListener('click', closeZoom));
			addCleanup(() => zoomImageContainer.removeEventListener('click', onOverlayClick));
			addCleanup(() => {
				if (zoomImageContainer.parentNode) zoomImageContainer.parentNode.removeChild(zoomImageContainer);
			});
		};
		...
		try {
			if (typeof window === 'undefined' || !window.__disableInlineCodeTabs) initInlineCodeTabs();
		} catch (err) {
		...
		return () => {
			try {
				if (typeof window !== 'undefined' && window.__cleanupDocTabsRuntime) {
					window.__cleanupDocTabsRuntime();
					delete window.__cleanupDocTabsRuntime;
				}
			} catch (_) {}
			try {
	...
	return <article id="md-content" dangerouslySetInnerHTML={{ __html: html }} />;
};
