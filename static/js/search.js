(function () {
	...
	function wire($) {
		...
		$.back &&
			$.back.addEventListener('click', function () {
				...
				var g = q('#gsc-i-id1, input.gsc-input');
				if (g) g.value = '';
				showDocs($);
	...
	function showResults($) {
		$.grid && $.grid.classList.add('hide');
		$.wrap && $.wrap.classList.remove('hide');
	}
	function showDocs($) {
		$.wrap && $.wrap.classList.add('hide');
		$.grid && $.grid.classList.remove('hide');
		$.hero && $.hero.classList.remove('hide');
	}
	...
	function gcseCallbacks($) {
		...
		window.__gcse.initializationCallback = function () {
			var g = q('#gsc-i-id1, input.gsc-input');
			if (g && !g.placeholder) g.setAttribute('placeholder', 'Search');
		};
	...
	function loadCSE(cx) {
		...
		s.setAttribute('data-gcse-loaded', 'true');
	...
	function retry(query, n, d) {
		var t = 0;
		(function go() {
			if (exec(query) || fallback(query)) return;
			if (++t >= n) return;
			setTimeout(go, d * Math.min(t + 1, 5));
		})();
	}
