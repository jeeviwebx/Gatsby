function sitevisitorTracking() {
	...
	var modificationurl = ['https://app.boldid.net/'];
	...
	const leadcategory = () => {
		...
		if (leadcategoryvalue == '' || leadcategoryvalue == null) {
			...
			const isYouTubeAds = utmSourceLower === 'youtube_ads' || utmCampaignLower.includes('youtube_ads');
	...
	const setlocalstorage = () => {
		...
		if (
			localStorage.getItem('secondaryreferralurl') == null ||
			(localStorage.getItem('secondaryreferralurl') != null && second_referralorgin().secondreferralorginUrl.toString().indexOf(subdomain) !== -1)
		) {
		...
		appendQueryParam =
			...
			'&referrerroriginurl=' +
			referralurlvalue +
			'&secondaryreferraloriginurl=' +
			scoundaryreferralurlvalue +
			'&leadcategory=' +
			localStorage.getItem('lead_category') +
			'&opportunitysource=' +
			localStorage.getItem('opportunity_source') +
			'&opportunitycategory=' +
			localStorage.getItem('opportunity_category') +
			'&uuid=' +
			uidValue +
			'&secondaryleadsource=Bold_Reports';
	};
