...
(function () {
	...
	function renderMarkdown(md) {
		...
		// Blockquotes
		md = md.replace(/(^|\n)>\s?(.*?)(?=\n|$)/g, function (m, p1, content) {
			return p1 + '<blockquote>' + content + '</blockquote>';
		});
...
`;
	...
	function restoreStoredMessages() {
		...
		storedMessages.forEach(function (m) {
			...
			try {
				applyChatMessageStyles(wrapper);
			} catch (e) {
				/* ignore */
			}
		});
	...
	function removeProcessing() {
		...
		msgs.forEach((m) => {
			const p = m.closest('.hb-msg');
			if (p) p.remove();
		});
		// remove any persisted processing entries
		storedMessages = storedMessages.filter(function (m) {
			return !/hb-processing/.test(m.html);
		});
		saveStoredMessages();
	}
	async function callApi(query) {
		try {
			...
			// Success response
			return data?.answer || 'Sorry, we couldn’t find an answer for your question. Please try rephrasing it.';
		} catch (err) {
